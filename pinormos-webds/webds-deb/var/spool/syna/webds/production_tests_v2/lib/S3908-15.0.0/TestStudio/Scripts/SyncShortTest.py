## DO NOT MODIFY
## 666cc85c5ab9bae2cc139305294f54ee959c4791-1.0.0.7
## DO NOT MODIFY
## Metadata:
# <?xml version="1.0" encoding="utf-8"?>
# <metadata name="Sync Short Test"
#       description="Sync Short Test (FW variable 'HSYNC trigger Mode Enable' needs to be 1 for this test to work.)"
#       bin="46" product="S3908">
#       <parameter name="Limits" type="string[][]" description="Limits for Sync Short Test"
#                   isoptional="false"
#                   islimit = "true"
#                   hint="Go to global settings to import test limit file." />
#       <parameter name="Sync Pin Locations" type="string[]" description="Sync Pin Locations"
#                   hint="Enter locations as comma separated values."
#                   isoptional="false">
#                       <default>0,1</default>
#       </parameter>
# </metadata>
...
class ReportBasedTest(object):
    def __init__(self):
        ...
        self._sync_pin_locations = []   # determined by schematics
    ...
    def __get_sync_locations(self):
        sync_locations = Comm2Functions.GetInputParam("Sync Pin Locations")
        if sync_locations is None:
            raise TestException("Sync pin locations not defined.")

        self._sync_pin_locations = []
        for pin in sync_locations:
            self._sync_pin_locations.append(int(pin.strip().strip("'").strip("\"")))
    ...
    def get_required_vars(self):
        ...
        """
        self.num_rows = 1
        self.num_cols = 16   # 2 byte vector
    ...
    def check_hsync_status(self):
        static_packet = Comm2Functions.Comm2DsCore_CreatePacket()
        try:
            static_cfg_helper = Comm2Functions.Comm2DsCore_GetHelper("staticConfiguration")
            if static_cfg_helper is None:
                Comm2Functions.Trace("Missing static configuration packet helper.")
                return None

            if Comm2Functions.Comm2DsCore_ExecuteCommand(0x21, [], static_packet) != 0:
                Comm2Functions.Trace("Cannot read static configuration from device.")
                return None

            h_sync_enabled = None

            temp = Comm2Functions.Comm2DsCore_GetVarValues(static_packet, "staticConfiguration", "syncEnabled")
            if temp is not None and len(temp) == 1:
                Comm2Functions.Trace("Reading syncEnabled from static config...")
                h_sync_enabled = temp[0]
                Comm2Functions.Trace("syncEnabled = {0}".format(h_sync_enabled))
                return " syncEnabled = {0}".format(int(h_sync_enabled) is 1)

            temp = Comm2Functions.Comm2DsCore_GetVarValues(static_packet, "staticConfiguration", "hSyncEnabled")
            if temp is not None and len(temp) == 1:
                Comm2Functions.Trace("Reading hSyncEnabled from static config...")
                h_sync_enabled = temp[0]
                Comm2Functions.Trace("hSyncEnabled = {0}".format(h_sync_enabled))
                return " hSyncEnabled = {0}".format(int(h_sync_enabled) is 1)

            if h_sync_enabled is None:
                Comm2Functions.Trace("Cannot read sync status variable.")
                return None
    ...
    def analyze_data(self):
        ...
        for row in range(0, self.num_rows):
            for column in range(0, self.num_cols):
                ...
                if column not in self._sync_pin_locations:
...
def main():
    ...
    try:
        ...
        status = test.check_hsync_status()
        ...
        if status is not None:
            Comm2Functions.SetStringResult(test.message + status)
        else:
        ...
        Comm2Functions.Trace(err.message)
        if status is not None:
            Comm2Functions.SetStringResult(err.message + status)
        else:
            Comm2Functions.SetStringResult(test.message)
        ...
        if status is not None:
            Comm2Functions.SetStringResult(exp + status)
        else:
            Comm2Functions.SetStringResult(test.message)
        Comm2Functions.SetTestResult(False)
    finally:
