## DO NOT MODIFY
## 8d94696b77f6ce204dfeb2fdc89c279bfb87a7cc-1.0.0.0
## DO NOT MODIFY
## Metadata:
# <?xml version='1.0' encoding='utf8'?>
# <metadata bin="67" description="Hybrid Raw Cap Test" name="Hybrid Raw Cap Test" product="S3913">
# <parameter description="Limits for Hybrid Raw Cap Test" hint="Go to global settings to import test limit file."
#       islimit="true" isoptional="false"
...
class ReportBasedTest(object):
    ...
    def get_static_config_info(self):
        ...
        if Comm2Functions.Comm2DsCore_ExecuteCommand(0x21, [], static_packet) != 0:
            ...
            self.num_abs_tx_rx = rx_count[0] + tx_count[0]
    ...
    def get_required_vars(self):
        ...
        try:
            ...
            temp = Comm2Functions.Comm2DsCore_GetVarValues(static_packet, self.STATIC_CONFIGURATION, self.ADNS_ENABLE)
            if temp is None:
                raise TestException("Cannot read {0} from device.".format(self.ADNS_ENABLE))

            if temp[0] != 1:
                raise TestException("ADNS is not enabled! Aborting test!")
    ...
    def get_result_matrix(self, matrices):
        total_avg = Comm2Functions.CreateMatrix(self.num_abs_tx_rx, 1)
        # get average of each tixel
        count = 0
        for matrix in matrices:
            count += 1
            for row in range(1):
                for col in range(self.num_abs_tx_rx):
                    total_avg[row][col] += matrix[row][col]

        for row in range(1):
            for col in range(self.num_abs_tx_rx):
                total_avg[row][col] /= count
    ...
    def run(self):
        ...
        try:
            ...
            if(len(matrices) > 1):
                self.result_matrix = self.get_result_matrix(matrices)
            else:
    ...
    def data_byte_array_to_matrix(self, byte_array):
        ...
        expected_len = self.num_abs_tx_rx * 2
        ...
        matrix = Comm2Functions.CreateMatrix(self.num_abs_tx_rx, 1)
        Comm2Functions.Trace("Adding frame to result matrix")
        i = 0
        for row in range(0, 1):
            for column in range(0, self.num_abs_tx_rx):
    ...
    def analyze_data(self):
        ...
        self.pass_fail_matrix = Comm2Functions.CreateMatrix(self.num_abs_tx_rx, 1)
    ...
    def prepare_multiple_limit_matrix(self):
        ...
        # Limit format is min,max; default limits are -400,400
        self.min_limit_matrix = Comm2Functions.CreateMatrix(self.num_cols - 1, 1)

        Comm2Functions.Trace("Creating max limit matrix...")
        self.max_limit_matrix = Comm2Functions.CreateMatrix(self.num_cols - 1, 1)

        Comm2Functions.Trace("Creating combined limit matrix...")
        self.limit_matrix = Comm2Functions.CreateMatrix(self.num_cols - 1, 1)

        for row in range(1):
            for column in range(self.num_cols - 1):
                idx = Comm2Functions.GetInputIndex("Limits", [row, column])
                try:
...
def raw_cap_report_data_converter(byte_array,
                                  image_row_num,
                                  image_col_num,
                                  word_size=2):
    if word_size != 2:
        raise TestException('currently only support 2-byte word size')
    converted_array = []
    indices = range(0, len(byte_array), word_size)
    for i in indices:
        # convert to 16-bit unsigned short
        short_val = unpack("<H",
                           unhexlify(str('%02x' % byte_array[i]) + str('%02x' % byte_array[i + 1])))[0]
        converted_array.append(short_val)
...
def main():
    ...
    test.report_id = 0x47
    test.name = 'Hybrid Raw Cap Test'
    test.data_collecting_type = 'production_test'
    test.add_compare_func('single', judgement_func)
    test.report_data_converter = raw_cap_report_data_converter
    ...
    try:
        ...
        xml_generator.set_row_headers(["{0}".format(element) for element in range(0, 1)])
        xml_generator.set_column_headers(["{0}".format(element) for element in range(0, test.num_abs_tx_rx)])
